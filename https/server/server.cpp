...
{
    if (argc < 2) {
        my::print_errors_and_exit("Need at least port");
    }
    ...
    if (SSL_CTX_use_PrivateKey_file(ctx.get(),  privateKeyPath.c_str(), SSL_FILETYPE_PEM) <= 0) {
        my::print_errors_and_exit("Error loading server private key");
    }

    auto accept_bio = my::UniquePtr<BIO>(BIO_new_accept(port.c_str()));
