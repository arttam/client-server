...
{
    if (argc < 2) {
        my::print_errors_and_exit("Need at least host:port to connect");
    }
    std::string hostPort{argv[1]};

    std::string request{"/"};
    if (argc > 2) {
        request.assign(argv[2]);
    }
    request.insert(0, "GET ");
    request.append(" HTTP/1.1");

    auto colonPos = hostPort.find_first_of(':');
    ...
    auto bio = my::UniquePtr<BIO>(BIO_new_connect(hostPort.c_str()));
    if (bio == nullptr) {
    ...
    my::send_http_request(bio.get(), request.c_str(), host.c_str());
